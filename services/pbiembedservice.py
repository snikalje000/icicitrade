...
from services.row_level_permission import PermissionConfig
...
class PbiEmbedService:
    ...
    def get_report_by_name(self, report_name, workspace_id):
        ...
        data_dict = json.loads(api_response.text)
        id = ""
        name = ""
        embedUrl = ""
        ...
        report = ReportConfig(id, name, embedUrl)

        return report.__dict__

    def get_embed_params_for_single_report_user(self, accessLevel, username, roles, datasets, workspace_id, report_id, report_name):
        perm_conf = PermissionConfig(accessLevel, [], username, roles, datasets)

        api_url = f"https://api.powerbi.com/v1.0/myorg/groups/{workspace_id}/reports/{report_id}/GenerateToken"
       
        api_response = requests.post(api_url, data=json.dumps(
            perm_conf.__dict__), headers=self.get_request_header())
        ...
        report = self.get_report_by_name(report_name, workspace_id)

        embed_config = EmbedConfig(
            api_response['tokenId'], api_response['token'], api_response['expiration'], [report])
